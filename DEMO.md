...
DO:
✓ Login with: test2@example.com / SecurePass456!@#
...
✓ Type: localStorage
✓ Press Enter
  → POINT OUT: Empty object {} or no auth-related keys
...
✓ Press Enter
  → POINT OUT: Only see non-HttpOnly cookies (XSRF token)
  → POINT OUT: Session cookie NOT visible (because HttpOnly)
...
✓ Expand files → Find main JavaScript bundle
✓ Ctrl+F search: "password"
  → POINT OUT: Only validation code, no credentials
...
After demo, have reviewers:
- [ ] Sign the reviewer form
- [ ] Mark each requirement as verified
- [ ] Provide any comments or concerns
- [ ] Take copy of signed checklist
